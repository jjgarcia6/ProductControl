...
Mecanismo transversal de auditoría append-only. El decorador `@audit(action, entity)` registra el
evento grueso de una operación (usuario, acción, entidad, objeto) sin diff campo-nivel;
`record_field_changes` registra el diff a nivel de campo de las correcciones (un registro por campo
modificado). `AuditAction` es el vocabulario canónico de acciones (`CREATE`, `UPDATE`, `CORRECTION`,
`SOFT_DELETE`, `STATE_CHANGE`). `AuditLog` es append-only: nunca se borra ni edita un registro.
