...
Gestión de listas de precios: catálogo de listas (con tipo `NORMAL` o `DESCARTE`), precios por par
(lista, producto) y baja lógica reversible (soft delete clase 2). Las listas se asignan a fichas de
tipo cliente del Directorio. La autorización fina se resuelve por el perfil del usuario (módulo
`PRICING`).
