...
Define la importación masiva de productos y de fichas de directorio a partir de archivos CSV o Excel:
previsualización sin persistir (dry-run), commit atómico all-or-nothing, deduplicación idempotente por
clave natural, validación delegada a las reglas de dominio de cada entidad, plantillas descargables,
validación del archivo (tipo, tamaño y límite de filas) y restricción de acceso por el módulo `bulk-import`.
